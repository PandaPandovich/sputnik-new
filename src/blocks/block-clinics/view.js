...
async function initMap() {
    ...
    if (maps) {
        maps.forEach((el) => {
            ...
            pinEl.innerHTML = '<svg width="32" height="42" viewBox="0 0 32 42" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                '<path d="M16 0C7.163 0 0 7.163 0 16c0 12 16 26 16 26s16-14 16-26C32 7.163 24.837 0 16 0z" fill="#FF4D4D"/>' +
                '<circle cx="16" cy="16" r="6" fill="#fff"/></svg>';
            pinEl.style.cssText = 'position:absolute;transform:translate(-50%,-100%);cursor:pointer;';

            map.addChild(new YMapMarker({ coordinates: [lng, lat] }, pinEl));
        })
    }
}