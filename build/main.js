...
(function () {
  ...
  input.addEventListener('input', function () {
    ...
    timer = setTimeout(() => {
      fetch(`${sputnikAjax.url}?action=sputnik_search&s=${encodeURIComponent(query)}`).then(r => r.json()).then(items => {
        if (!items.length) {
          results.innerHTML = `
...
(function () {
  ...
  input.addEventListener('input', function () {
    ...
    timer = setTimeout(function () {
      fetch(`${sputnikAjax.url}?action=sputnik_search&s=${encodeURIComponent(query)}`).then(function (r) {
        return r.json();
      }).then(function (items) {
